#!/bin/bash
#
# Pulls daily DWR TEST GOES17 solar and ETo grass mapsets to the local processor
# uses the existing GOES15 receiver as an ssh proxy
# should be run as the cimis user on the DWR processor(s)
TUNNELHOST=10.28.200.5                  # tunneling host at DWR
DWRHOST=10.158.241.77                   # DWR TEST CIMIS GOES17 processor
...
function get-solar ()
{
rsync -avz -e "ssh -i /home/cimis/.ssh/ucdgoes -A cimis@$TUNNELHOST ssh -i /home/cimis/.ssh/dwr-receiver" cimis@$DWRHOST:$SDIR/$day $SDIR
}
function get-eto ()
{
rsync -avz -e "ssh -i /home/cimis/.ssh/ucdgoes -A cimis@$TUNNELHOST ssh -i /home/cimis/.ssh/dwr-receiver" cimis@$DWRHOST:$CDIR/$day $CDIR
}
if [[ $1 = [0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9] ]];then
        ...
        echo
        echo "Usage:  g.cimis-pull-solar-dwr.sh [YYYYMMDD] [OPTION]"
        echo
        ...
        echo
        echo "Examples:"
        echo "  g.cimis                 pull yesterday's solar and eto data from DWR TEST GOES17."
