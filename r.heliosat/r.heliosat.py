...
our @R_mapcalc_common = 
  (
   ...
   latitude=> sub {pod2usage(msg=>"Specify latitude=[geocentric latitude]",exitval=>1);},
   ssha=> sub {pod2usage(msg=>"Specify ssha=[sunshine hour angle]",exitval=>1);},
   p_p0=>"exp(-#elevin#/8434.5)",
   hclinke=>"\"#linkein#\"*#p_p0#",
   ray=>"1/(6.6296+1.7513*#p_p0#-0.1202*#p_p0#^2+0.0065*#p_p0#^30.000013*#p_p0#^4)",
   slsd=>"sin(#latitude#)*sin(#declin#)",
   clcd=>"cos(#latitude#)*cos(#declin#)",
   );
...
pod2usage(msg=>"Specify year month day [hour] [minute] [timezone]",exitval=>1) unless ($year && $month && $day);
...
# Get some Daily input Parameters
{
    ...
    my %rsol;
    map ({chomp; my($n,$v)=split('=');$rsol{$n}=$v} `r.solpos -r year=$year month=$month day=$day hour=12 timezone=$timezone`);
...
for(my $i=0; $i<=$#R_mapcalc; $i+=2) {
    my $n=$R_mapcalc[$i];
    if ($isInstant{$n}) {
	$Rname{$n}||=sprintf "${prefix}$n%2.2d%2.2d",$hour,$minute;
    } else {
	$Rname{$n}||="${prefix}$n";
    }
}
